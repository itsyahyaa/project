...
def anotherWin():
    tkinter.messagebox.showinfo(
        "About", 'Driver Drowsiness Detection System version v1.0\n Made Using\n-OpenCV\n-Numpy\n-Tkinter\n In Python 3')
...
class StartPage(tk.Frame):
    ...
    def __init__(self, master):
        ...
        tk.Label(self, text="DRIVER\n DROWSINESS\n DETECTION SYSTEM\n", font=(
            'Times 35 bold'), background='light blue').pack(side="top")
...
class PageOne(tk.Frame):
    ...
    def __init__(self, master):
        ...
        label_0 = tk.Label(self, text="Registration form",
...
class PageTwo(tk.Frame):
    def __init__(self, master):
        ...
        Date = tk.StringVar()
        dateCheck = False
        tk.Frame.configure(self, background="#E0FFFF")

        tk.Label(self, text="WELCOME TO DDDS", font=(
            'Helvetica', 18, "bold")).pack(side="top", fill="x", pady=5)

        tk.Button(self, text='Start', height=5, width=30, bg='brown',
                  fg='red', command=cam).place(x=130, y=100)

        label_5 = tk.Label(self, text="Enter Date(YYYY-MM-DD)",
                           width=30, font=("bold", 10))
        label_5.place(x=130, y=210)

        entry_5 = tk.Entry(self, textvar=Date)

        entry_5.place(x=130, y=230)

        tk.Button(self, text="View Report", bg='brown', fg='red',
                  width=30, command=report).place(x=130, y=280)

        tk.Button(self, text="Send Email Report", bg='green', fg='red',
                  width=30, command=send_mail).place(x=130, y=330)

        tk.Button(self, text="Go back to start page", width=30,
                  command=lambda: master.switch_frame(StartPage)).place(x=130, y=380)
